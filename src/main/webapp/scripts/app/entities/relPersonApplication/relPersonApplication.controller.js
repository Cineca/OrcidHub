...
angular.module('huborcidApp')
    .controller('RelPersonApplicationController', function ($scope, RelPersonApplication, Person, Application, Token, ParseLinks) {
        ...
        $scope.loadAll = function() {
            RelPersonApplication.query({page: $scope.page, per_page: 20}, function(result, headers) {
        ...
        $scope.showUpdate = function (id) {
            RelPersonApplication.get({id: id}, function(result) {
        ...
        $scope.save = function () {
            if ($scope.relPersonApplication.id != null) {
                RelPersonApplication.update($scope.relPersonApplication,
                    function () {
                        $scope.refresh();
                    });
            } else {
                RelPersonApplication.save($scope.relPersonApplication,
                    function () {
                        $scope.refresh();
                    });
            }
        ...
        $scope.confirmDelete = function (id) {
            RelPersonApplication.delete({id: id},
                function () {
                    $scope.loadAll();
